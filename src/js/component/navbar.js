...
export const Navbar = () => {
    ...
    useEffect(() => {
        setNumFavorites(store.favorites.length);
    }, [store.favorites]);

    useEffect(() => {
        // Combina los datos de personajes, planetas y vehículos para las opciones de búsqueda
        const combinedOptions = [
            ...store.characters.map(character => ({ value: `/character/${character.uid}`, label: character.name, type: 'character' })),
            ...store.planets.map(planet => ({ value: `/planet/${planet.uid}`, label: planet.name, type: 'planet' })),
            ...store.vehicles.map(vehicle => ({ value: `/vehicle/${vehicle.uid}`, label: vehicle.name, type: 'vehicle' }))
        ];
        setSearchOptions(combinedOptions);
    }, [store.characters, store.planets, store.vehicles]);

    const handleSearchSelect = (selectedOption) => {
        // Navegar a la página detallada del elemento seleccionado
    ...
    const handleAddToFavorites = (character) => {
        if (!store.favorites.find((fav) => fav.uid === character.uid)) {
            ...
            localStorage.setItem('favorites', JSON.stringify([...store.favorites, character]));
        } else {
    ...
    const handleRemoveFromFavorites = (index) => {
        ...
        setNumFavorites(numFavorites - 1); // Disminuye el contador de favoritos
        // Elimina el favorito del almacenamiento local
		...
		<nav className="navbar bg-dark d-flex">
			<div className="container-fluid ">
				<a className="navbar-brand" href="/">
					<svg xmlns="http://www.w3.org/2000/svg" width="100" height="50" viewBox="0 0 634.105 271.365">
                ...
                <div className="dropdown">
                    <button className="btn btn-warning dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                        Notifications <span className="badge text-bg-danger">{numFavorites}</span>
                    </button>
                    <ul className="dropdown-menu dropdown-menu-end">
                        {store.favorites.map((favorite, index) => (
                            <li key={index} className='d-flex justify-content-between p-1 align-items-center'><a className="dropdown-item">{favorite.name} 
							</a> <button className='btn btn-light' onClick={handleRemoveFromFavorites}><i className="fa-solid fa-trash" style={{color: '#000000'}}></i></button></li>
                        ))}
					</ul>
				</div>
			</div>
