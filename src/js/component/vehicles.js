...
export const Vehicles = () => {
    ...
    const handleAddToFavorites = (vehicle) => {
        ...
        if (!updatedFavorites.find((fav) => fav.name === vehicle.name)) {
            // Añadir el nuevo personaje a la lista de favoritos actualizada
            updatedFavorites.push(vehicle);
                    ...
                    {store.vehicles.map((vehicle, index) => (
                        <SwiperSlide key={index}>
                            <div className="card" style={{ width: '18rem' }}>
                                ...
                                <a href={`/vehicle/${vehicle.uid}`} className="btn btn-warning">Learn more</a>
                                    <button className='btn btn-dark' onClick={() => handleAddToFavorites(vehicle)}>
                                    <LikeBtn />
                                </button>
                                </div>
                            </div>
